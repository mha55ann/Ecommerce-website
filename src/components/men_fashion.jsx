...
import Rating from "./SideCompnents/Rating"; // Make sure this import is present
...
function Men_fashion() {
  ...
  const handleClick = (id) => {
    navigate("/details", {
      state: { array: menData.filter((item) => item.id === id) },
    });
  };
  const handleCartClick = (e, id) => {
    e.stopPropagation(); // Prevents navigation to details
    addToCart(menData.find((item) => item.id === id));
    navigate("/cart", {
      state: { array: menData.filter((item) => item.id === id) },
    });
  };

  const { menData, menError, menLoading, addToCart } = useContextApi();

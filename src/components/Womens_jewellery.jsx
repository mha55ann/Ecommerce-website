...
function Womens_jewellery() {
  ...
  const handleClick = (id) => {
    navigate("/details", {
      state: { array: womensJewelleryData.filter((item) => item.id === id) },
    });
  };
  const handleCartClick = (e, id) => {
    e.stopPropagation(); // Prevents navigation to details
    const product = womensJewelleryData.find((item) => item.id === id);
    if (product) {
  ...
  if (womensJewelleryError) {
    return (
      <p className="text-center text-xl font-semibold text-red-500">
        Error: {womensJewelleryError.message}
      </p>
    );
  }
  ...
  return (
    <div className="bg-[#e9ebee] py-10">
      ...
      <div className="flex flex-wrap justify-center gap-8 px-4">
        {womensJewelleryData.map((product) => (
          <div
            key={product.id}
...
export default Womens_jewellery;

...
function Fragrances() {
  ...
  const { fragrancesData, fragrancesError, fragrancesLoading, addToCart } =
    useContextApi();
  const handleClick = (id) => {
    navigate("/details", {
      state: { array: fragrancesData.filter((item) => item.id === id) },
    });
  };
  const handleCartClick = (e, id) => {
    e.stopPropagation(); // Prevents navigation to details
    const product = fragrancesData.find((item) => item.id === id);
    if (product) {

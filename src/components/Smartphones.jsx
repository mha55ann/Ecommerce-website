...
function Smartphones() {
  ...
  const { smartphonesData, smartphonesError, smartphonesLoading, addToCart } =
    useContextApi();
  const handleClick = (id) => {
    navigate("/details", {
      state: { array: smartphonesData.filter((item) => item.id === id) },
    });
  };
  ...
  if (smartphonesError) {
    return (
      <p className="text-center text-xl font-semibold text-red-500">
        Error: {smartphonesError.message}
      </p>
    );
  }
        ...
        {smartphonesData.map((product) => (
          ...
          >
            ...
            <div className="p-4">
              ...
              <button onClick={(e) => handleCartClick(e, product.id)} className="w-full bg-[#4f39f6] text-white px-4 py-2 rounded hover:bg-[#3d2bc7] transition font-semibold">
                Add to Cart
              </button>
            </div>
          </div>
